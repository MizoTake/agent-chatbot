...
test('DiscordAdapter.buildSlashCommands: 表示する slash command は代表名に絞る', () => {
  const adapter = new DiscordAdapter('dummy-token');
  const commands = (adapter as any).buildSlashCommands();
  const names = commands.map((command: any) => command.name);

  assert.deepEqual(names, [...DISPLAYED_SLASH_COMMANDS]);
  assert.equal(names.some((name: string) => name.startsWith('codex-') && name !== 'codex-model'), false);
});
...
test('DiscordAdapter.downloadIncomingImageAttachments: 受信画像をツール参照用にローカル保存する', async () => {
  const adapter = new DiscordAdapter('dummy-token');
  const downloadIncomingImageAttachments = (adapter as any).downloadIncomingImageAttachments.bind(adapter);
  const originalFetch = globalThis.fetch;

  (globalThis as any).fetch = async () => new Response(Buffer.from([0x89, 0x50, 0x4e, 0x47]), {
    status: 200,
  ...
  try {
    const attachments = await downloadIncomingImageAttachments([
      { url: 'https://cdn.example.com/frame.png', name: 'frame.png', contentType: 'image/png' },
      { url: 'https://cdn.example.com/spec.pdf', name: 'spec.pdf', contentType: 'application/pdf' }
    ], 'C001', 'M001');

    assert.equal(attachments.length, 1);
    assert.equal(attachments[0].kind, 'image');
    assert.equal(attachments[0].fileName, 'frame.png');
    assert.equal(attachments[0].contentType, 'image/png');
    assert.equal(attachments[0].url, 'https://cdn.example.com/frame.png');
    assert.ok(attachments[0].path);
    assert.ok(fs.existsSync(attachments[0].path!));
    savedPath = attachments[0].path;
  } finally {
