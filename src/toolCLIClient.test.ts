...
test('parseToolOutput: codex SSE タイムアウトのエラーメッセージを返す', () => {
  const client = new ToolCLIClient({
    codex: { command: 'codex', args: ['exec', '{prompt}'], versionArgs: ['--version'] }
  }, 'codex', 5000);
  ...
  try {
    ...
    const jsonl = [
      '{"type":"thread.started","thread_id":"tid_789"}',
      '{"type":"turn.started"}',
      ...
      '{"type":"turn.failed","error":{"message":"stream disconnected before completion: idle timeout waiting for SSE"}}'
    ].join('\n');

    const result = parse(tool, jsonl);
    assert.ok(result.response.includes('codex エラー'));
    assert.ok(result.response.includes('idle timeout'));
    assert.equal(result.sessionId, 'tid_789');
  } finally {
    client.cleanup();
  }
});

test('isTransientToolError: SSE タイムアウトはリトライ可能と判定する', () => {
  const client = new ToolCLIClient({}, 'claude', 5000);

  try {
    const check = (client as any).isTransientToolError.bind(client);
    assert.equal(check('⚠️ codex エラー: stream disconnected before completion: idle timeout waiting for SSE'), true);
    assert.equal(check('⚠️ codex エラー: connection refused'), true);
    assert.equal(check('正常な応答テキスト'), false);
